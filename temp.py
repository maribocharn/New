...
def masss(n): #функция с одним входным параметром (количество массивов)
    ...
    for i in range(n): #заводим цикл для главного массива
        f=rd.randint(1, 100) #генерируем случайное число (длина будущего подсписка)
        while f in proverochka: #если подсписок такого размера уже есть
            f=rd.randint(1, 100) #генерируем случайное число снова, пока размерность не перестанет совпадать с уже существующеми длинами подсписков
...
#ограничим наш массив натуральными числами до 100 для наглядности списка
n=int(input('Введите количество желаемых массивов (натуральное число не больше 100 для наглядности): ')) 
while n<=0 or n>100:
    n=int(input('Введенное Вами число не является натуральным или больше 100. Попробуйте еще раз: '))
out=masss(n)
